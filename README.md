...
- a lower or equal rank (higher or similar importance): creating a new parser instance with the new rank, which continues and returns an appropriate leftValue
- a higher rank (lower importance): which will return the rightValue immediately, along with the resulting expression list which is used to update the previous level to reflect the resulting values that weren't consumed
